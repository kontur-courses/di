...
{
    public class AnglesCloudLayouter : ITagCloudLayouter
    {
        private List<Rectangle> previousRectangles;
        private SortedList<double, List<(Point point, int countNear)>> pointForAdd;
        private Point center;

        public AnglesCloudLayouter()
        {
        ...
        public Rectangle PutNextRectangle(Size rectangleSize)
        {
            ...
            for (var j=0; j< keys.Count; j++)
            {
                ...
                {
                    var angle = pointForAdd[distanceToCenter][i];
                    foreach (var corner in GetCornerPositions(rect))
                    {
                        var LeftTopPosition = new Point(angle.point.X - corner.X, angle.point.Y - corner.Y);
                        rect.MoveToPosition(LeftTopPosition);
                        if (!IntersectsWithPrevious(rect))
                        {
                            AddRectangle(rect);
                            angle.countNear += 1;
                            if (angle.countNear == 2)
                            {
