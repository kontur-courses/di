...
public class FlowerSpiralTests
{
    ...
    {
        Action action = () => new FlowerSpiral(center, -1, -1);
        action.Should().Throw<ArgumentException>()
            .WithMessage($"petalCount or petalLength must not be less than 0");
    }

    [TestCaseSource(nameof(ConstructorSpiralPoints))]
    public void FlowerSpiral_GetNextPoint_CreatePointsWithCustomAngle_ReturnsCorrectPoints(double petalLength, int petalCount, float angleStep, 
        Point[] expectedPoints)
    {
        var spiral = new FlowerSpiral(center,petalLength, petalCount, angleStep);
