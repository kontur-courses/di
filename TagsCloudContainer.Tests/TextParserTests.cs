...
namespace TagsCloudVisualization.Tests
{
    ...
    {
        private readonly HashSet<string> stopWords = new HashSet<string> {"a", "not", "to", "an", "are"};
        private TextParser Parser { get; set; }

        [SetUp]
        public void SetUp()
        {
            Parser = new TextParser(stopWords);
        }
        ...
        private static IEnumerable<TestCaseData> TestCases()
        {
            ...
            yield return new TestCaseData("12 34", new Dictionary<string, int> {{"12", 1}, {"34", 1}})
                .SetName("Digits");
            ...
            yield return new TestCaseData("not a words", new Dictionary<string, int> {{"words", 1}}).SetName(
                "Stop words");
